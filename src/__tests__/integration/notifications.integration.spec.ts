...
describe('Notifications — intégration HTTP', () => {
  ...
  beforeAll(async () => {
    ...
    ({ app } = await createTestApp({
      ...
      prismaOverride: {
        ...prisma,
        sMSNotification: { findMany: smsHistoryMock },
  ...
  describe('POST /api/notifications/sms/send', () => {
    it('201 — ADMIN enfile un SMS manuel → jobId retourné', async () => {
      const res = await request(app.getHttpServer())
        .post('/api/notifications/sms/send')
        .set('Authorization', `Bearer ${ADMIN_TOKEN}`)
        .send({ phone: '+237690000000', message: 'Votre véhicule est prêt' });
    ...
    it('enfile le job "manual-sms" avec les bons paramètres', async () => {
      await request(app.getHttpServer())
        .post('/api/notifications/sms/send')
        .set('Authorization', `Bearer ${ADMIN_TOKEN}`)
        .send({ phone: '+237690000000', message: 'Test', customerId: 'cust-1', lang: 'en' });

      expect(queueAddMock).toHaveBeenCalledWith('manual-sms', {
        phone: '+237690000000',
        message: 'Test',
        customerId: 'cust-1',
        lang: 'en',
