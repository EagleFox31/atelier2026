        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-foreground">{invoice.reference}</h1>
            ...
            <p className="text-muted-foreground text-sm">
              ...
              {invoice.serviceOrder && <> · <span className="font-mono">{invoice.serviceOrder.reference}</span></>}
            </p>
          </div>
        </div>
        <div className="flex gap-2 print-hide">
          <Button variant="outline" className="gap-2 border-border" onClick={() => router.push(`/billing/invoices/${id}/print`)}>
            ...
            <Dialog open={isPaymentOpen} onOpenChange={setIsPaymentOpen}>
              ...
              <DialogContent className="sm:max-w-[440px] bg-card border-border">
                ...
                <div className="space-y-4 pt-2">
                  ...
                  >
                    {paymentLoading ? 'Enregistrement...' : `Valider le paiement · ${formatXAF(Number(paymentAmount) || 0)}`}
                  </Button>
                </div>
              </DialogContent>
            </Dialog>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border-border shadow-sm">
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-lg">{customerName(invoice.customer)}</CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground space-y-1">
            {invoice.customer?.phonePrimary && <p>{invoice.customer.phonePrimary}</p>}
            ...
            {payments.map((p: any, i: number) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                  <div className="flex items-center gap-3">
                    {method && <method.icon size={18} className="text-muted-foreground" />}
                    <div>
                      <p className="text-sm font-medium text-foreground">{method?.label ?? p.method}</p>
                      {p.transactionRef && (
                        <p className="text-xs font-mono text-muted-foreground">{p.transactionRef}</p>
                      )}
                    </div>
                  </div>
